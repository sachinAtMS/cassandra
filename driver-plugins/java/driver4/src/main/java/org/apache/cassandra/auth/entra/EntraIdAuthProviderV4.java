...
/**
 * DataStax Java Driver 4.x {@link AuthProvider} for Microsoft Entra ID token-based authentication.
 *
 * <p>This provider acquires a JWT access token from Microsoft Entra ID using MSAL4J
 * and sends it to Apache Cassandra's {@code EntraIdAuthenticator} via the SASL PLAIN mechanism.
 * The 4.x driver's auth interface is fully asynchronous — token acquisition runs in a
 * background thread so it never blocks the driver's event loop.</p>
 *
 ...
 * }</pre>
 *
 * <h3>application.conf (typesafe config):</h3>
 * <pre>
 * datastax-java-driver {
 *   advanced.auth-provider {
 *     class = org.apache.cassandra.auth.entra.EntraIdAuthProviderV4
 *   }
 * }
 * </pre>
 *
...
{
    private static final Logger logger = LoggerFactory.getLogger(EntraIdAuthProviderV4.class);
    ...
    /** Returns the configured username, or {@code null} if not using ROPC. */
    ...
    {
        logger.debug("Creating Entra ID authenticator for endpoint {} (server authenticator: {})",
    ...
    {
        // Not all Cassandra versions send an initial challenge — this is fine
        logger.trace("No initial challenge from {}", endPoint);
    }

    @Override
    public void close() throws Exception
    {
        // MSAL4J application instances don't need explicit cleanup
    }
    ...
    /**
     * Asynchronous SASL PLAIN authenticator for the 4.x driver.
     *
     * <p>All methods return {@link CompletionStage} to support the driver's async model.
     * Token acquisition is offloaded to {@code CompletableFuture.supplyAsync} so it
     * never blocks the driver's I/O threads.</p>
     */
    ...
    /**
     * Builder for constructing {@link EntraIdAuthProviderV4} instances.
     */
    public static class Builder
    {
        ...
        /** Sets the OAuth2 scope. Default: {@code api://{clientId}/.default} */
        public Builder scope(String scope) { this.scope = scope; return this; }

        /** Sets the authority URL. Default: {@code https://login.microsoftonline.com/{tenantId}} */
        public Builder authority(String authority) { this.authority = authority; return this; }

        /** Enables Azure Managed Identity. Use when on Azure VMs, App Service, AKS, etc. */
